...
2. 提交代码到 `main`。
3. 推送到远端。
4. Render 拉取 `main` 后开始部署。
5. 由用户在远程主站完成验证，不要求先做本地测试。
...
### 6.3 Auto-Deploy 恢复验证结论
- 验证提交 `6f24679` 已从 GitHub `main` 自动部署到 Render，站内“设置与诊断”显示同一短号。
- 回退提交 `f8ecb77` 也已自动部署成功，说明自动部署不是单次偶发成功。
- 本次问题排查中，曾出现“本地有提交但未真正推到 GitHub”导致 Render 无法触发部署的情况，需先区分 GitHub 推送状态与 Render 部署状态。
- 当前可视为 `GitHub main -> Render Auto-Deploy -> 线上运行版本` 链路已恢复正常。
